...
import { ChatMessage, ImageContentPart, MessageBubble, TextContentPart } from "./Config/Interfaces";
...
export default function Home() {
  ...
  const sendMessage = async (messageToSend: string, currentModel: string, system?: string, images?: File[]) => {
    ...
    if (images && images.length > 0) {
      imageUrls = await Promise.all(images.map((file) => apiService.convertFileToBase64(file)));
    }
    ...
    const { newContextMessages, calculatedLimit } = await createContext(CONTEXT_METHOD.SLIDING_WINDOW, updatedChat);

    console.log("Context sent to api", newContextMessages);

    const result = await apiService.sendMessage(messageToSend, currentModel, onChunk, temperature, system, images, newContextMessages);
  ...
  const createContext = async (method: CONTEXT_METHOD, currentChat: MessageBubble[]) => {
    ...
    switch (method) {
      ...
      case CONTEXT_METHOD.SLIDING_WINDOW:
        for (const [index, msgObj] of currentChat.slice(-parseInt(CONTEXT_METHOD.SLIDING_WINDOW as any)).entries()) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 items-center justify-end min-h-screen font-sans dark:from-zinc-950 dark:to-zinc-900 gap-8 p-6">
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 flex flex-row-reverse items-end justify-center w-full max-w-4xl gap-3 p-3 mx-auto">
          <div className="shrink-0 mb-2">
            <ContextMonitor context={contextLimit} />
          </div>

          {/* Główny kontener ma teraz ZAWSZE flex-col, dzięki czemu zdjęcia są zawsze na górze */}
          <div className="flex flex-col gap-3 w-full max-w-3xl p-3 rounded-2xl bg-white/70 dark:bg-zinc-900/70 backdrop-blur-md border border-white/20 dark:border-zinc-800/50 shadow-xl shadow-slate-200/50 dark:shadow-none">

            {/* Sekcja zdjęć – zawsze na samej górze */}
            ...
            {/* Dolny kontener na kontrolki – zachowuje Twoją płynną animację i logikę row/col */}
            <div className={`transition-all duration-200 w-full flex ${
              ...
              <div className={isExpanded ? 'order-2 flex items-center justify-between w-full' : 'contents'}>
